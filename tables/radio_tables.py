...
def multiple_mass(table, conn, cursor, file_number):
    ...
    if table == "SonnoMammography_Multiple_Mass":
        sonno_quad, sonno_location, sonno_clock, sonno_shape,  sonno_depth, sonno_distance, sonno_pect,sonno_orientation, sonno_margin, sonno_echo, \
        sonno_posterior = [list([]) for _ in range(11)]
    ...
    for index in range(0, mass_number):
        ...
        while not check:
            ...
            if table == "Mammography_Multiple_Mass":
                ...
                mass_echo = ask_y_n_statement.ask_option("Echo pattern of mass "+ str(mass_id), ["Anechoic", "Hyperechoic",
                            "Complex cystic and solid", "Hypoechoic", "Isoechoic", "Heterogeneous"])
                sonno_echo.append(mass_echo)
                mass_posterior = ask_y_n_statement.ask_option("Posterior Acoustic features", ["No posterior features", "Enhancement",
                ...
                data_list = [file_number, mass_id, mass_location, location_quad, location_clock, mass_depth,
                             mass_location,mass_dist, mass_shape, mass_orientation, mass_margin, mass_echo, mass_posterior]
...
def cal_table (file_number, conn, cursor):
    ...
    location, quad, depth, dist, pect, type, calc_type, dist= [list([]) for _ in range(8)]
    for index in range(0, mass_number):
        check = False
        while not check:
            mass_id = index + 1
    ...
    all_data = [[str(mass_number)], location, quad, depth, dist, pect, type, calc_type, dist]
    data_return = ask_y_n_statement.join_lists(all_data, "; ")
    return tuple(data_return)

def lesion_location(lesion, category = ["Location on Right Breast", "Location on Left Breast"],
                    option = ["UOQ", "UIQ","UCQ", "LOQ","LIQ","LCQ", "COQ", "CIQ","CCQ"]):
    import modules.ask_y_n_statement as ask_y_n_statement
    lesion_data =[]
