...
def mammography (conn, cursor, file_number):
    ...
    import sql.add_update_sql as add_update_sql
    import modules.pccm_names as pccm_names
    import tables.radio_tables as radio_tables
    ...
    while not check:
        ...
        if sonomammo:
            ...
            if sonomammo_sol_duct:
                sol_duct_loc = ask_y_n_statement.ask_option("Location of Solitary Dilated duct", ["Right Breast", "Left Breast", "Both"])
            ...
            sonomammo_date, sonomammo_acc, sonomammo_tissue, mass_number_sonomammo, mass_location, mass_quad, mass_clock, \
            mass_depth, mass_distance, mass_pect, mass_shape_sonomammo, mass_orientation_sonomammo, mass_margin_sonomammo,\
            mass_echo_sonomammo, mass_posterior_sonomammo, sonomammo_calc, sonomammo_calc_type, sonomammo_arch,\
            sonomammo_duct, sonomammo_skin, sonomammo_edema, sonomammo_vasc, sonomammo_elast, sonomammo_lymph_intra, \
            sonomammo_lymph_ax, lymph_ax_cort, lymph_ax_hilum, lymph_ax_vasc, sol_duct_loc, sol_duct_diam, sol_mass, \
            sonomammo_other, sonomammo_shear, sonomammo_vtq, sonomammo_birad, sonomammo_diag = ("NA", )*36
        data_list = [sonomammo, sonomammo_date, sonomammo_acc, sonomammo_tissue, mass_number_sonomammo,
                     mass_location, mass_quad, mass_clock, mass_depth,mass_distance,mass_pect,mass_shape_sonomammo,
                     mass_orientation_sonomammo, mass_margin_sonomammo, mass_echo_sonomammo, mass_posterior_sonomammo,
                     ...
                     lymph_ax_hilum, lymph_ax_vasc, sol_duct_loc, sol_duct_diam, sol_mass, sonomammo_shear,
                     sonomammo_vtq, sonomammo_other, sonomammo_birad, sonomammo_diag]
        columns_list = pccm_names.name_clinical(module_name)
        check = add_update_sql.review_input(file_number, columns_list, data_list)
    return (tuple(data_list))
...
def add_data(conn, cursor, file_number):
    ...
    if enter:
        ...
        add_update_sql.update_multiple(conn, cursor, table, pccm_names.name_clinical("nipple_cytology"), file_number,
                                       data)
    enter = ask_y_n("Enter results of other clinical tests?")
    if enter:
...
def edit_data(conn, cursor, file_number):
    ...
    col_list = colname.name_clinical("other_test")
    enter = add_update_sql.review_data(conn, cursor, table, file_number, col_list)
    if enter:
        data = other_test(file_number)
        add_update_sql.update_multiple(conn, cursor, table, col_list, file_number, data)
