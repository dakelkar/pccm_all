    ...
    while not check_tox:
        ...
        if tox:
                ...
                while add_tox:
                    ...
                    tox_grade = ask_y_n_statement.ask_option(("the grade of " + tox_present + "? "), ["Mild",
                                                                                                      "Moderate",
                                                                                                      "Severe",
                                                                                                      "Other"])
                    treatment = input(
                    ...
                    check = False
                    while not check:
                        data = [file_number, drug_cyc, tox_present, tox_grade, treatment, resp_treatment, cyc_tox,
                                change_tox]
...
def drug_table_enter(file_number, drug_table):
    ...
    while drug_add:
        ...
        while not check_drug:
            ...
            dose = input("Dose of " + drug + " (actual dose amount without unit, numbers only): ")
            dose_unit = 'NA'
            if dose != 'NA':
                try:
                    float(dose)
