...
import additional_tables.block_description as block_data
from datetime import datetime


def surgery_block_information_1(file_number, cursor, block_data_type='surgery_1'):
    ...
    while not check:
        ...
        lesion_side = ask.ask_option("Lesion on", ["Right Breast", "Left Breast", "Bilateral"])
        try:
                sql = ('SELECT NACT_status FROM Neo_Adjuvant_Therapy WHERE File_number = \'' + file_number + "'")
                cursor.execute(sql)
                nact_ = cursor.fetchall()
                nact = nact_[0][0]
        except:
        ...
        check = add_update_sql.review_input(file_number, columns_list, data_list_1)
    return (tuple(data_list_1))

def surgery_block_information_2(file_number, cursor):
    module_name = "surgery_block_information_2"
    check = False
    while not check:
        try:
                sql = ('SELECT Number_Blocks_Surgery_Block FROM Surgery_Block_Report_Data WHERE File_number = \'' + file_number + "'")
                cursor.execute(sql)
                no_blocks_ = cursor.fetchall()
                no_blocks = no_blocks_[0][0]
        except:
            no_blocks = input("Please input number of blocks (if block information is not available please enter 0: ")
        try:
                sql = ('SELECT Block_ID_Surgery_Block FROM Surgery_Block_Report_Data WHERE File_number = \'' + file_number + "'")
                cursor.execute(sql)
                block_id_ = cursor.fetchall()
                block_id = block_id_[0][0]
        except:
            block_id = input("Please input block id: ")
        block_desc_df, block_data_all = block_data.block_description(file_number, block_id, no_blocks)
        tumour_size = input("Tumour size (please input digits only): ")
        tumour_unit = input ("Tumour size unit: ")
        tumour_grade = ask.ask_option("Tumour Grade", ["1", "2", "3"])
        ...
        per_inv = ask.ask_y_n_na("Perineural Invasion", yes_ans='Yes', no_ans='No', na_ans='Data not in Report')
        necrosis = ask.ask_y_n_na("Necrosis", yes_ans='Yes', no_ans='No', na_ans='Data not in Report')
        percent_vasc_invasion = input("Percent Vascular Invasion (Enter 'Data not in report' if not available): ")
        percent_lymph_invasion = input("Percent Lymphocyte Invasion (Enter 'Data not in report' if not available): ")
        percent_stroma = input("Percent Stroma (Enter 'Data not in report' if not available): ")
        ...
        report = ask.ask_option("Pathological Complete Remission", ["Yes", "No", 'PCR not mentioned in report'])
        ...
        data_list = [tumour_block_ref, node_block_ref, ad_normal_block_ref, red_tissue_block_ref,tumour_size, tumour_unit,
                     ...
                     percent_lymph_invasion, percent_stroma, margin, margin_id, margin_type, report]
        columns_list = names(module_name)
