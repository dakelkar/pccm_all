...
def surgery_information(file_number):
    ...
    while not check:
        ...
        surgery_hospital = ask.ask_list("Name of hospital", SurgeryLists.hospitals)
        ...
        surgery_name_anaesthetist = ask.ask_list("Name of Anaesthetist", SurgeryLists.anesthetists)
        surgery_name_surgeon = ask.ask_list("Name of the Surgeon/s", SurgeryLists.surgeon)
        ...
        if surgery_incision == 'Circum-areolar':
            type = ask.ask_list('Pleaes define margine', ['At areolar margin', 'Away from margin', 'Data not in report'])
            surgery_incision = surgery_incision + ': ' +  type
        surgery_type = ask.ask_list("Types of Surgery", ["Conservative Breast Surgery", "Implant Based Reconstruction",
                                                      'Requires Specialist Input'])
        if surgery_type == "Conservative Breast Surgery":
            surgery_type_subtype = ask.ask_list("Type of Breast Conservation Surgery", ['Conventional Surgery',
                                                                                      'Oncoplastic Surgery', 'Other'])
            if surgery_type_subtype == "Conventional Surgery":
                surgery_type_level_subtype = ask.ask_list("type of conventional surgery",
                ...
                surgery_type_level_subtype = ask.ask_option("Level of oncoplastic surgery",
                                                            SurgeryLists.oncoplastic_levels)
                if surgery_type_level_subtype == "Level 1":
                    oncoplastic_surgery_type = ask.ask_list("Type of level 1 oncoplastic surgery",
                                                            SurgeryLists.level_1_type)
                    if oncoplastic_surgery_type == "Type 2: Volume Displacment":
                        oncoplastic_surgery_flap = ask.ask_list("Type of flap used", SurgeryLists.level_2_flap)
                        oncoplastic_surgery_plan, oncoplastic_surgery_tumour_filled_by, oncoplastic_surgery_nac_graft, \
                    ...
                    oncoplastic_surgery_type = ask.ask_list("Subtypes of Therapeutic Mammoplastty", ["Simple "
                                                                                                     "Therapeutic",
                                                                                                     "Extreme "
                                                                                                     "Therapeutic"])
                    oncoplastic_surgery_flap = "Therapeutic Mammoplasty"
                    if oncoplastic_surgery_type == "Simple Therapeutic":
                        oncoplastic_surgery_plan = ask.ask_list("Type of Plan used", SurgeryLists.simple_plan)
                        ...
                        oncoplastic_surgery_primary_pedicle = ask.ask_list("Type of pedicle used",
                    ...
                    if oncoplastic_surgery_type == "Extreme Therapeutic":
                        oncoplastic_surgery_tumour_filled_by = ask.ask_list("Tumour filled by",
                                                                            ["Extended Primary Pedicle",
                                                                             "Secondary Pedicle"])
                        oncoplastic_surgery_plan = ask.ask_list("Type of Plan used", SurgeryLists.simple_plan)
                        oncoplastic_surgery_nac_graft = ask.ask_y_n("Is NAC graft done?")
                        if not oncoplastic_surgery_nac_graft:
                            oncoplastic_surgery_nac_graft = "NAC not grafted "
                            oncoplastic_surgery_primary_pedicle = ask.ask_list("What is Nipple On (primary pedicle) "
                                                                                 "pedicle used?",
                                                                                 SurgeryLists.primary_pedicle)
                            oncoplastic_surgery_secondary_pedicle = ask.ask_list("What is the secondary pedicle used?",
...
def post_surgery(file_number, user_name):
    ...
    while not check:
        ...
        radio = ask_option_y_n("Is radiotherapy planned?", "Radiotherapy planned", "No Radiotherapy")
