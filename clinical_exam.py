    ...
    while not check:
        ...
        if mammo:
            ...
            if mammo_mass_location:
                ...
                mass_number, mammo_mass_location, mammo_mass_location_quad, mammo_mass_shape, mammo_mass_margin, \
                mammo_mass_density = radio_tables.multiple_mass(table, conn, cursor, file_number)
            else:
                mass_number, mammo_mass_location, mammo_mass_location_quad, mammo_mass_shape, mammo_mass_margin, \
                mammo_mass_density = ("NA", )*6
            calc = ask_y_n_statement.ask_y_n("Is Calcification present?")
            if calc:
                calc_number, calc_location, location_quad, calc_name, calc_type, calc_dist \
                    = radio_tables.cal_table(file_number, conn, cursor)
            else:
                calc_number, calc_location, location_quad, calc_name, calc_type, calc_dist = ("NA", )*6
            ...
            if mammo_arch:
                ...
                mammo_arch, arch_loc, arch_quad,  = ("NA", )*3
            ...
            if asym:
                ...
                mammo_asymm = radio_tables.lesion_location(asym_loc,["Type of Asymmetry in Right Breast", "Type of Asymmetry in Left Breast"],
                                                           ["Asymmetry","Global asymmetry","Focal asymmetry",
                                                        "Developing asymmetry", "Other"])
            else:
                asym_loc,asym_quad, mammo_asymm = ("NA", )*3
            mammo_intra = input("Description of intramammary lymph nodes: ")
            ...
            for index in (asso_feat):
                ...
                var = ask_y_n_statement.ask_option(index, ["Right Breast", "Left Breast", "Both", "Not Present", "Other"])
            ...
            mammo_depth = ask_y_n_statement.ask_option("Depth of the lesion?", ["Anterior", "Middle", "Posterior", "Other"])
            mammo_lateral = ask_y_n_statement.ask_option("Lateral location of lesion", ["Right Breast", "Left Breast", "Both", "Not Present", "Other"])
            if mammo_lateral in {"Right Breast", "Left Breast", "Both"}:
                mammo_quad = radio_tables.lesion_location(mammo_lateral)
            else:
                mammo_quad = "NA"
            mammo_dist = ask_y_n_statement.ask_option("Distance from nipple", ["<0.5 cm", ">0.5 cm"])
            ...
            mammo_mass_shape, mammo_mass_margin, mammo_mass_density, calc_number, calc_location, location_quad, \
            calc_name, calc_type, calc_dist, mammo_arch, arch_loc, arch_quad, asym_loc,asym_quad, mammo_asymm, \
            mammo_intra,  mammo_lesion, mammo_dil, asso_feat_1, asso_feat_2, asso_feat_3,asso_feat_4, asso_feat_5, \
            asso_feat_6, asso_feat_7, mammo_depth, mammo_lateral,mammo_quad,mammo_dist, mammmo_pect, mammo_birad, \
            mammo_diag = ("NA",) * 38
        data_list = [mammo_loc, mammo_details, mammo_date, mammo_number, mammo_rep_previous, mammo, mammo_diag_date,
                     ...
                     mammo_mass_shape, mammo_mass_margin, mammo_mass_density, calc_number, calc_location, location_quad,
                     calc_name, calc_type, calc_dist, mammo_arch, arch_loc, arch_quad, asym_loc,asym_quad,
    ...
    while not check:
        ...
        if tomo:
            ...
            if tomo_lesion in {"Right Breast", "Left Breast", "Both"}:
                tomo_lesion_loc  = radio_tables.lesion_location(tomo_lesion)
            else:
                tomo_lesion_loc = "NA"
    ...
    while not check:
        ...
        if sonomammo:
            ...
            if other:
                ...
                sonomammo_other = "NA"
            sonomammo_dist = ask_y_n_statement.ask_option("Distance from Nipple", ["< 5cm", "> 5 cm", "Other"])
            sonamammo_skin_dist =  ask_y_n_statement.ask_option("Distance from Skin", ["< 0.5cm", "> 0.5 cm", "Other"])
            ...
            sonomammo_date, sonomammo_acc, sonomammo_tissue, mass_sonomammo, mass_location,
            mass_quad, mass_clock, mass_number_sonomammo, mass_shape_sonomammo, mass_orientation_sonomammo,
            mass_margin_sonomammo, mass_echo_sonomammo, mass_posterior_sonomammo, sonomammo_calc,
            sonomammo_calc_type, sonomammo_arch, sonomammo_duct, sonomammo_skin, sonomammo_edema,
            sonomammo_vasc, sonomammo_elast, sonomammo_lymph_intra, sonomammo_lymph_ax, lymph_ax_cort,
            lymph_ax_hilum, lymph_ax_vasc, sonomammo_other, sonomammo_birad, sonomammo_diag,
            sonomammo_dist, sonamammo_skin_dist, sonammo_pect = ("NA", )*32
        data_list = [sonomammo, sonomammo_date, sonomammo_acc, sonomammo_tissue, mass_sonomammo, mass_number_sonomammo,
                     mass_location, mass_quad, mass_clock, mass_shape_sonomammo, mass_orientation_sonomammo,
                     mass_margin_sonomammo, mass_echo_sonomammo, mass_posterior_sonomammo, sonomammo_calc,
                     sonomammo_calc_type,  sonomammo_arch, sonomammo_duct, sonomammo_skin, sonomammo_edema,
                     sonomammo_vasc, sonomammo_elast, sonomammo_lymph_intra, sonomammo_lymph_ax, lymph_ax_cort,
